...
In reality, many of our reactions were more complex than this. We even created our own reaction, which supportes fetching data from the server automatically when certain dependencies change. For example, changing filters in the UI caused the application to get new data from the server.
