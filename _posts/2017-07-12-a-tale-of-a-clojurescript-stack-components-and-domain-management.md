...
In reality, many of our reactions were more complex than this. We even created our own reaction, which supported fetching data from a server automatically when certain dependencies changed. For example, changing filters in the UI caused the application to get new data from the server.
