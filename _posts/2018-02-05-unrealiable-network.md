...
In most cases, however, it is not practical to re-try every single failed request. For exampole, if a request pointed to a file which was not found and returned 404, it is probably not useful to try this request again. Thus, you should choose which requests you might want to try again.
